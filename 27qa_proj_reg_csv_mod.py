...
# -*- coding: utf-8 -*-
"""
Created on Mon Jun 26 19:10:50 2023
...
#------------------------------------------------------------------------------
# Goal here is to append the correlation values of Observation (Obs) and Multi-
# model mean(MMM) to the csv file containing the values for the model.
...
for region in regions:
    ...
    #with open(wd+'mmm_Arry_reg_proj_'+season+'_'+region+'.csv', 'r') as file_mmm:
     #   reader_c = csv.reader(file_mmm)        #read in mm file and extract row

        #for _ in range(1):
            #next(reader_c)
                 #extract 1st row from mmm_file
        # Skip rows until you reach the desired row



    with open('immm_regV2proj_'+season+'_'+region+'.csv', 'r') as file_mmm:
        reader_c = csv.reader(file_mmm)        #read in mm file and extract row

        for _ in range(1):
            next(reader_c)
        msource_row  = next(reader_c)         #extract 1st row from mmm_file

    
    with open('immmcat1_regV2proj_'+season+'_'+region+'.csv', 'r') as file_mcat1:
    ...
    with open('immmcat2_regV2proj_'+season+'_'+region+'.csv', 'r') as file_mcat2:
    ...
    existing_data = []
    with open('iArry_proj_reg_'+region+'_'+season+'.csv', 'r') as file_b:
    ...
    new_file_name = 'iAllproj_reg_'+region+'_manuscript.csv'  # Specify the new file name
