...
#                label='historical', color='royalblue')
        plt.bar(bar_positions_hist, hist_values, width=bar_width,
                label='historical', color='royalblue')
        plt.bar(bar_positions_proj, proj_values, 
                width=bar_width, label='ssp585 projections', color='red')   #yerr=proj_stde, capsize=1,
...
#        cat1_line = lines.Line2D([], [], color='blue', ls='dotted',
#                                 label='Cat1 models')
        band_patch = mpatches.Patch(color='darkred', alpha=0.15, label='Obs std error')
        legend_patches = [mpatches.Patch(color='royalblue', label='Historical \u03b2'),
                          mpatches.Patch(color='red', label='SSP585 projected \u03b2'),
                          obs_line, band_patch]
