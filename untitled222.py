...
for i, region in enumerate(regions):
    ...
    if DRAW_SCATTERPLOT:
        ...
        for i, row in enumerate(df_combined.iterrows()):
            ...
            axs[i // 2, i % 2].text(row[1]['b'], row[1]['pri_std']+0.02, str(i + 1),
                     color='blue', fontsize=15, ha='center', va='center')

        # Plot projected data
        for i, row in enumerate(df_combined.iterrows()):
            axs[i // 2, i % 2].scatter(row[1]['b_proj'], row[1]['pri_std_proj'], c='red', alpha=0.7)
            axs[i // 2, i % 2].text(row[1]['b_proj'], row[1]['pri_std_proj']+0.01, str(i + 1),
        ...
        if region == "Central_Africa":
            ...
            axs[i // 2, i % 2].plot(hist_x_filtered, hist_slope_filtered * hist_x_filtered + hist_intercept_filtered,
            ...
            axs[i // 2, i % 2].text(x_position, y_position, f'\u03b2 obs = {obs_bcoef:.2f}', fontsize=18, rotation=90, va='center')
            axs[i // 2, i % 2].text(xr_position, yr_position, f'rstd = {obs_pri_stdev: .2f}', fontsize=18)
            axs[i // 2, i % 2].text(xcor_position, ycor_position, f'r (Historical) = {corr_coefficient_hist:.2f},\n p-val (Historical) = {p_value_hist:.3f}', fontsize=18, color='blue')
        ...
        axs[i // 2, i % 2].tight_layout()  # Ensures all elements are visible
