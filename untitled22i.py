...
#fig, axs = plt.subplots(2, 2, figsize=(46, 38), dpi=100)
    ...
    if DRAW_SCATTERPLOT:
        ...
        if region == "Central_Africa":
            ...
            axs[i // 2, i % 2].plot(hist_x_filtered, hist_slope * hist_x_filtered + hist_intercept,
                     ...
                     label=f'Regression Line (Hist - with outliers) y = {hist_slope:.2f}x + {hist_intercept:.2f}')
            axs[i // 2, i % 2].plot(hist_x, hist_slope_n * hist_x + hist_intercept_n, color='green',
                     label=f'Regression Line (Hist - No outliers) y = {hist_slope_n:.2f}x + {hist_intercept_n:.2f}')
